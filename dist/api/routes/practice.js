...
practiceRoutes.post('/log', async (c) => {
    try {
        ...
        const { bpm, rhythm, rhythmPattern, scale, tonality, position, notePattern, key } = body;
        ...
        if (rhythm && scale && position) {
            ...
            const scaleSig = { dimension: 'scale', scale };
            const positionSig = { dimension: 'position', position };
            const notePatternSig = {
                dimension: 'note-pattern',
                pattern: notePattern || 'stepwise',
            };
            const entry = engine.logCompoundPractice(rhythmSig, scaleSig, tonality || 'minor', positionSig, notePatternSig, key || 'C', bpm);
            return c.json({
                success: true,
                entry: {
                    id: entry.id,
            ...
            const suggestion = engine.getLastSuggestion();
            if (!suggestion) {
                return c.json({ success: false, error: 'No suggestion to log. Generate a suggestion first.' }, 400);
            }
...
practiceRoutes.put('/history/:id', async (c) => {
    try {
        ...
        repo.updatePractice(id, rhythmSig, { dimension: 'scale', scale }, tonality || 'minor', { dimension: 'position', position }, { dimension: 'note-pattern', pattern: notePattern || 'stepwise' }, key || 'C', bpm, npm);
        // Recalculate stats after update
        repo.recalculateAllStats(settings.emaAlpha, settings.progression.expansionNpm, settings.progression.masteryNpm, settings.progression.masteryStreak, settings.npmTiers.struggling);
