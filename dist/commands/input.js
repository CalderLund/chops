...
function showStrugglingWarning(engine, settings, compoundStats) {
    if (!compoundStats)
        return;
    ...
    if (compoundStats.strugglingStreak >= streakThreshold) {
        ...
        if (compoundStats.strugglingStreak === 1) {
            ...
            console.log(chalk.red(`  ${compoundStats.strugglingStreak} attempts below ${settings.npmTiers.struggling} NPM`));
        }
...
function logLastSuggestion(engine, repo, settings, bpm) {
    try {
        const suggestion = engine.getLastSuggestion();
        if (!suggestion) {
            console.error(chalk.red('Error: No suggestion to log. Run "chops" first to get a suggestion.'));
            process.exit(1);
        }
        // Track unlocks before logging
        const unlocksBefore = repo.getUnlockedDimensions();
        // Log using compound system
        const entry = engine.logCompoundPractice(suggestion.rhythm, suggestion.scale, suggestion.tonality, suggestion.position, suggestion.notePattern, suggestion.key, bpm, suggestion.reasoning);
        ...
        console.log(chalk.gray(`  Scale:    ${entry.scale.scale} ${entry.tonality}`));
        ...
        if (compoundStats) {
            ...
            if (compoundStats.isMastered) {
                ...
                if (compoundStats.masteryStreak > 0) {
                    console.log(chalk.magenta(`  Mastery streak: ${compoundStats.masteryStreak}/${settings.progression.masteryStreak}`));
                }
                else {
                ...
                console.log(chalk.yellow(`  Expansion progress: ${Math.round(progress * 100)}% (need ${settings.progression.expansionNpm} NPM)`));
            }
        }
...
async function interactiveInput(engine, repo, settings, bpmArg) {
    ...
    try {
        ...
        const defaultTonality = lastSuggestion?.tonality ?? 'minor';
        const tonalityResponse = await enquirer.prompt({
            type: 'select',
            name: 'tonality',
            message: 'Tonality',
            choices: tonalities,
            initial: tonalities.indexOf(defaultTonality),
        });
        ...
        const rhythm = { dimension: 'rhythm', rhythm: rhythmResponse.rhythm, pattern };
        const scale = { dimension: 'scale', scale: scaleResponse.scale };
        const position = { dimension: 'position', position: positionResponse.position };
        const notePattern = { dimension: 'note-pattern', pattern: selectedNotePattern };
        // Track unlocks before logging
        const unlocksBefore = repo.getUnlockedDimensions();
        const entry = engine.logCompoundPractice(rhythm, scale, tonalityResponse.tonality, position, notePattern, keyResponse.key, bpm, 'Custom practice');
