...
export async function strugglingCommand(repo, engine, settings, options) {
    ...
    try {
        ...
        if (strugglingCompounds.length === 0) {
            ...
            for (const compound of strugglingCompounds) {
                ...
                console.log(`  ${chalk.red('•')} ${parts.join(' + ')} ${chalk.gray(`(${compound.strugglingStreak} attempts)`)}`);
            }
            console.log();
        }
        ...
        if (actionResponse.action === 'view-tiers') {
            ...
            console.log(`  ${chalk.yellow('Developing')}:    ${settings.npmTiers.struggling} - ${settings.npmTiers.developing - 1} NPM`);
            console.log(`  ${chalk.blue('Progressing')}:   ${settings.npmTiers.developing} - ${settings.npmTiers.progressing - 1} NPM`);
            console.log(`  ${chalk.cyan('Fast')}:          ${settings.npmTiers.progressing} - ${settings.npmTiers.fast - 1} NPM`);
            console.log(`  ${chalk.green('Very Fast')}:     ${settings.npmTiers.fast} - ${settings.npmTiers.veryFast - 1} NPM`);
            console.log(`  ${chalk.magenta('Super Fast')}:    ${settings.npmTiers.veryFast} - ${settings.npmTiers.superFast - 1} NPM`);
...
//# sourceMappingURL=struggling.js.map