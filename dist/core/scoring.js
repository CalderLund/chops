...
export function scoreCandidate(rhythm, scale, position, notePattern, context) {
    const { settings } = context;
    let score = 0;
    // Count changes from previous
    const rhythmChanged = rhythm.rhythm !== context.previousRhythm.rhythm ||
    ...
    const changes = (rhythmChanged ? 1 : 0) + (scaleChanged ? 1 : 0) + (positionChanged ? 1 : 0) + (notePatternChanged ? 1 : 0);
    // Proximity bonus
    if (changes === 1) {
    ...
    const avgDaysSince = (rhythmDaysSince + scaleDaysSince + positionDaysSince + notePatternDaysSince) / 4;
...
function isSignatureStable(stats, settings) {
    if (!stats)
        return false;
    return isStable(stats.attempts, stats.emaNpm, stats.bestNpm, settings.stability.minAttempts, settings.stability.emaRatio);
}
function daysSinceLastSeen(stats, now) {
...
export function weightedRandomSelect(items, scores, random = Math.random) {
    if (items.length === 0) {
