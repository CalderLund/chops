...
export function createDatabase(dbPath) {
    ...
    const practiceLogExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='practice_log'").get();
...
function ensureDefaultUser(db) {
    ...
    if (!existing) {
        db.prepare("INSERT INTO users (name, created_at) VALUES ('default', ?)").run(new Date().toISOString());
    }
}
// Get or create a user by name, returns user ID
export function getOrCreateUser(db, name) {
    const existing = db.prepare("SELECT id FROM users WHERE name = ?").get(name);
    if (existing) {
        return existing.id;
    }
    const result = db.prepare("INSERT INTO users (name, created_at) VALUES (?, ?)").run(name, new Date().toISOString());
    return result.lastInsertRowid;
}
// Get user ID by name (returns null if not found)
export function getUserId(db, name) {
...
function migrateToMultiUser(db) {
    ...
    const practiceInfo = db.prepare("PRAGMA table_info(practice_log)").all();
  ...
  `);
    ...
    const oldStatsExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='signature_stats'").get();
    if (oldStatsExists) {
        const statsInfo = db.prepare("PRAGMA table_info(signature_stats)").all();
      ...
      `);
          ...
          VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(stat.signature_id, stat.dimension, stat.best_npm ?? 0, stat.ema_npm ?? 0, stat.attempts ?? 0, stat.last_seen ?? null, stat.has_expanded ?? 0, stat.mastery_streak ?? 0, stat.is_mastered ?? 0);
            }
        }
    }
    // Migrate compound_stats
    const oldCompoundExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='compound_stats'").get();
    if (oldCompoundExists) {
        const compoundInfo = db.prepare("PRAGMA table_info(compound_stats)").all();
      ...
      `);
            // Restore data with user_id = 1
            for (const comp of oldCompounds) {
                db.prepare(`
          ...
          VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(comp.id, comp.scale, comp.position, comp.rhythm, comp.rhythm_pattern, comp.note_pattern ?? null, comp.articulation ?? null, comp.best_npm ?? 0, comp.ema_npm ?? 0, comp.attempts ?? 0, comp.has_expanded ?? 0, comp.mastery_streak ?? 0, comp.is_mastered ?? 0, comp.last_practiced ?? null, comp.last_practiced_session ?? null);
            }
        }
    }
    // Migrate dimension_unlocks
    const oldUnlocksExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='dimension_unlocks'").get();
    if (oldUnlocksExists) {
        const unlocksInfo = db.prepare("PRAGMA table_info(dimension_unlocks)").all();
      ...
      `);
            for (const unlock of oldUnlocks) {
                db.prepare(`
    ...
    const oldSessionExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_counter'").get();
    if (oldSessionExists) {
        const sessionInfo = db.prepare("PRAGMA table_info(session_counter)").all();
        const sessionHasUserId = sessionInfo.some(c => c.name === 'user_id');
        if (!sessionHasUserId) {
            const oldSession = db.prepare("SELECT current_session FROM session_counter WHERE user_id = 1").get();
      ...
      `);
            if (oldSession) {
                db.prepare("INSERT INTO session_counter (user_id, current_session) VALUES (1, ?)").run(oldSession.current_session);
            }
        }
    }
...
function runMigrations(db) {
    ...
    const sessionCounterExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_counter'").get();
    if (sessionCounterExists) {
        const sessionRow = db.prepare('SELECT current_session FROM session_counter WHERE user_id = 1').get();
        if (!sessionRow) {
            // Initialize session counter based on practice log count
            const countRow = db.prepare('SELECT COUNT(*) as count FROM practice_log').get();
            const practiceCount = countRow?.count ?? 0;
            db.prepare('INSERT OR REPLACE INTO session_counter (id, current_session) VALUES (1, ?)').run(practiceCount);
        }
    }
    // Migration 1b: Add tonality column to practice_log
    const practiceLogInfo = db.prepare("PRAGMA table_info(practice_log)").all();
    ...
    const compoundStatsExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='compound_stats'").get();
    ...
    const tableInfo = db.prepare("PRAGMA table_info(practice_log)").all();
    const columns = tableInfo.map((c) => c.name);
    // Migration 3: Add streaks and achievements tables
    const streaksTableExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='streaks'").get();
    ...
    `);
        ...
        const fretboardStats = db.prepare("SELECT * FROM signature_stats WHERE dimension = 'fretboard'").all();
        for (const stat of fretboardStats) {
            ...
            if (parts.length === 3) {
                ...
                const existingScale = db.prepare("SELECT * FROM signature_stats WHERE signature_id = ?").get(scaleId);
                if (existingScale) {
                    db.prepare(`
          ...
          `).run(scaleId, stat.best_npm, stat.ema_npm, stat.attempts, stat.last_seen);
                ...
                const existingPosition = db.prepare("SELECT * FROM signature_stats WHERE signature_id = ?").get(positionId);
                if (existingPosition) {
                    db.prepare(`
            UPDATE signature_stats
            SET best_npm = MAX(best_npm, ?), ema_npm = ?, attempts = attempts + ?, last_seen = ?
