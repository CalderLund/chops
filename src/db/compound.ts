...
export function compoundId(compound: Compound): string {
  const parts = [compound.scale, compound.position, `${compound.rhythm}:${compound.rhythmPattern}`];
...
export function parseCompoundId(id: string): Compound {
...
export function compoundsEqual(a: Compound, b: Compound): boolean {
  return (
    a.scale === b.scale &&
...
export function getChangedDimension(from: Compound, to: Compound): string | null {
  const changes: string[] = [];

  if (from.scale !== to.scale) changes.push('scale');
  if (from.position !== to.position) changes.push('position');
  if (from.rhythm !== to.rhythm || from.rhythmPattern !== to.rhythmPattern) changes.push('rhythm');
  if (from.notePattern !== to.notePattern) changes.push('note-pattern');
  if (from.articulation !== to.articulation) changes.push('articulation');

  return changes.length === 1 ? changes[0] : null;
}

// Count how many dimensions differ between two compounds
export function countDimensionChanges(from: Compound, to: Compound): number {
  let count = 0;

  if (from.scale !== to.scale) count++;
  if (from.position !== to.position) count++;
  if (from.rhythm !== to.rhythm || from.rhythmPattern !== to.rhythmPattern) count++;
  if (from.notePattern !== to.notePattern) count++;
  if (from.articulation !== to.articulation) count++;

  return count;
}

// Get the active dimension count for a compound
export function getActiveDimensionCount(compound: Compound): number {
  let count = 3; // scale, position, rhythm always active
  if (compound.notePattern !== undefined) count++;
  if (compound.articulation !== undefined) count++;
  return count;
}

// Create entry point compound for given unlocked tiers
export function createEntryCompound(
  scaleEntry: string,
  positionEntry: string,
  rhythmEntry: string,
  rhythmPatternEntry: string,
  notePatternEntry?: string,
  articulationEntry?: string,
): Compound {
  const compound: Compound = {
    scale: scaleEntry,
...
export function statsToCompound(stats: CompoundStats): Compound {
  const compound: Compound = {
    scale: stats.scale,
