...
graphRoutes.get('/layout', async (c) => {
  try {
    ...
    const { repo, settings, dimensions, engine } = getContext(user);
    ...
    if (suggest) {
      try {
        ...
        recommendedReasoning = suggestion.reasoning;
      } catch {
        // If suggestion generation fails, just return graph without recommendation
      }
    }
