...
import { Settings, parseSigId, SignatureStats } from '../types.js';

export function statsCommand(repo: Repository, settings: Settings): void {
  ...
  console.log(
    chalk.gray(`      Example: 80 BPM × 2 (8ths) = 160 NPM, 80 BPM × 4 (16ths) = 320 NPM`),
  );
  console.log(chalk.gray(`  • Compounds = Scale + Position + Rhythm combinations`));
  console.log(
    chalk.gray(
      `  • ${chalk.yellow('practicing')} = Below expansion threshold (${settings.progression.expansionNpm} NPM)`,
    ),
  );
  console.log(
    chalk.gray(
      `  • ${chalk.cyan('expanded')}   = Hit ${settings.progression.expansionNpm}+ NPM, neighbors unlocked`,
    ),
  );
  console.log(
    chalk.gray(
      `  • ${chalk.magenta('streak N/3')} = N consecutive practices at ${settings.progression.masteryNpm}+ NPM`,
    ),
  );
  console.log(
    chalk.gray(
      `  • ${chalk.green('MASTERED')}  = ${settings.progression.masteryStreak} consecutive at ${settings.progression.masteryNpm}+ NPM`,
    ),
  );
...
function displayCompoundStats(repo: Repository, settings: Settings): void {
  ...
  console.log(
    chalk.gray(
      `  Total: ${compounds.length} compounds, ${expanded} expanded, ${mastered} mastered`,
    ),
  );
}

function displayDimensionProgress(repo: Repository, settings: Settings): void {
  console.log(chalk.yellow.bold('\n  DIMENSION UNLOCKS\n'));
  ...
  if (notePatternUnlocked) {
    ...
    const needed =
      settings.dimensionTiers.find((d) => d.name === 'note-pattern')?.unlockRequirement ?? 5;
    console.log(
      chalk.yellow(`  ○ Note Pattern`) +
        chalk.gray(` (Tier 1 - ${expanded}/${needed} compounds expanded)`),
    );
  }
  ...
  if (articulationUnlocked) {
    ...
    const needed =
      settings.dimensionTiers.find((d) => d.name === 'articulation')?.unlockRequirement ?? 5;
    console.log(
      chalk.gray(`  ○ Articulation`) +
        chalk.gray(` (Tier 2 - ${expanded}/${needed} compounds expanded)`),
    );
  } else {
    console.log(
      chalk.gray(`  ○ Articulation`) + chalk.gray(` (Tier 2 - unlock Note Pattern first)`),
    );
  }
}

function displayLegacyStatsCompact(stats: SignatureStats[], _settings: Settings): void {
  ...
  for (const s of sorted) {
    ...
    console.log(
      chalk.gray(
        `    ${status} ${padRight(sigName, 15)} ${s.attempts} attempts, ${Math.round(s.bestNpm)} NPM`,
      ),
    );
  }
}
