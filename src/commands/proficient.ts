...
export async function proficientCommand(
  repo: Repository,
  engine: Engine,
  ...
  try {
    ...
    if (allProficiencies.length === 0) {
      ...
      for (const dim of options.dimensions) {
        const values = allProficiencies.filter((p) => p.dimension === dim.name).map((p) => p.value);
        if (values.length > 0) {
    ...
    if (actionResponse.action === 'add') {
      ...
      const toAdd = [valueResponse.value, ...prerequisites].filter(
        (v) => !currentProficiencies.includes(v),
      );
