...
import { Suggestion } from '../db/suggestion.js';
import { Engine } from '../core/engine.js';
import { Repository } from '../db/repository.js';
import { Settings } from '../types.js';

export function exerciseCommand(engine: Engine, repo: Repository, settings: Settings): void {
...
function displayDimensionUnlock(dimensions: string[], settings: Settings): void {
...
function displayCompoundProgress(repo: Repository, settings: Settings): void {
...
export function displaySuggestion(suggestion: Suggestion): void {
  const line = '━'.repeat(45);
  ...
  console.log(
    `  ${chalk.gray('Scale:')}     ${formatScale(suggestion.scale.scale)}`,
  );
  console.log(`  ${chalk.gray('Position:')}  ${suggestion.position.position}-shape`);
  console.log(
    `  ${chalk.gray('Rhythm:')}    ${suggestion.rhythm.rhythm} (${suggestion.rhythm.pattern})`,
  );
